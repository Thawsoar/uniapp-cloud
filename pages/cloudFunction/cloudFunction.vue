<template>
	<view class="content">
		<unicloud-db ref="udb" v-slot:default="{data, loading, error, options}" collection="unicloud-test">
			<uni-table :loading="loading" :emptyText="error.message || '没有更多数据'" border stripe >
				<uni-tr>
					 <uni-th>name</uni-th>
					 <uni-th>subType</uni-th>
					 <uni-th>createTime</uni-th>
				 </uni-tr>
				 <uni-tr v-for="(item,index) in data" :key="index">
					 <uni-th>{{item.name}}</uni-th>
					 <uni-td>{{item.subType}}</uni-td>
					 <uni-td>{{parseTime2(item.createTime)}}</uni-td>
				 </uni-tr>
			 </uni-table>
		</unicloud-db>
